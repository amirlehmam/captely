...
def process_enrichment_batch(self, file_path: str, job_id: str, user_id: str, use_ultra_fast: bool = True):
    ...
    if cache_source in ["cache_user_duplicate", "cache_global"]:
        ...
        # Save cached result to contacts table
        try:
            with SyncSessionLocal() as session:
                # Handle credits for cache hits
                if credits_to_charge > 0:
                    # Check credits before charging
                    credits_check = httpx.get(
                        f"http://credit-service:8000/api/credits/balance/{user_id}",
                        timeout=5.0
                    )
                    
                    if credits_check.status_code == 200:
                        available_credits = credits_check.json().get("credits", 0)
                    ...
                    # Charge credits for global cache hits (user still pays, we save API cost)
                    if cache_source == "cache_global":
                        reason_parts = []
                ...
                # Insert contact record with cache data
                contact_insert = text("""
                    INSERT INTO contacts (
                        job_id, first_name, last_name, company, position, location, 
                ...
                # Check if job is complete and trigger notifications if needed
                # [Job completion logic here - same as before]
...
def cascade_enrich(self, lead: Dict[str, Any], job_id: str, user_id: str, enrichment_config: Dict[str, bool] = None):
    ...
    # 🚀 SAVE TO CACHE - Store successful API results for future optimization
    if enrichment_successful and (contact_data.get("email") or contact_data.get("phone")):
        try:
            ...
            if cache_save_result.get("cache_id"):
                print(f"   📎 Cache ID: {cache_save_result['cache_id']}")
                print(f"   💰 Future cost savings: ${cache_save_result.get('potential_savings', 0):.3f} per reuse")
                
        except Exception as cache_error:
            print(f"⚠️ Cache save failed (non-critical): {cache_error}")
            # Don't fail the enrichment if cache save fails
